...
import {
  ACTIVITY_TIMELINE, BUILD_STEPS, DRIVER_PHASES, DRIVING_QUESTION, EMERGENCY_CONTEXT,
  MATCH_TYPES, MISSION, NAV_ITEMS, OFFICIAL_MATCH_TYPES, RELIEF_ROLES, START_RULES,
  TEAM_COLORS, createTeamScore, createTimer, makeId,
} from './data/challenge';
  ...
  return (
    <main id="main-content" className="home-page">
      <section className="hero">
        <div className="hero-copy"><span className="eyebrow">The Science Exchange Program at LSTS</span><h1>VEX Rapid<br /><em>Innovation</em> Challenge</h1><p className="hero-tagline">Build. Practice. Improve. Compete.</p><p className="driving-question"><b>Driving Question:</b> {DRIVING_QUESTION}</p><p className="mission-line">{MISSION}</p><div className="hero-actions"><button className="primary-action" type="button" onClick={() => goTo('teams')}>Set Up Teams <span aria-hidden="true">→</span></button><button className="text-action" type="button" onClick={() => goTo('mission')}>View mission</button></div></div>
...
function Mission({ acknowledged, acknowledge, openMedia, goTo }) {
  const officialField = { src: './images/field.jpg', title: 'Official Challenge Field', alt: 'Challenge field showing the Neutral Zone, three team zones, delivery zones and robot starts' };
  ...
  return (
    <main id="main-content" className="mission-page page-shell"><header className="page-lead mission-story-lead"><span className="step-tag">02 · Mission & Field</span><span className="driving-label">Driving Question</span><h1>{DRIVING_QUESTION}</h1><p>{EMERGENCY_CONTEXT}</p></header>
      <section className="emergency-brief" aria-labelledby="emergency-mission-title"><div className="emergency-mission"><span className="eyebrow">Emergency response mission</span><h2 id="emergency-mission-title">Adapt. Transport. Deliver.</h2><p>{MISSION}</p></div><dl className="relief-roles">{RELIEF_ROLES.map(([object, role]) => <div key={object}><dt>{object}</dt><dd>{role}</dd></div>)}</dl></section>
      <section className="field-stage"><div className="field-heading"><div><span className="eyebrow">Official layout</span><h2>Three response teams share one field</h2></div><button type="button" className="secondary-action" onClick={() => openMedia(officialField)}>View Full Field</button></div><button className="field-image-button" type="button" onClick={() => openMedia(officialField)} aria-label="Open full field map"><img src="./images/field.jpg" alt="Challenge field with a shared Neutral Zone, three team zones, robot starts and delivery zones" /><span>Tap to enlarge</span></button></section>
      ...
      <section className="scoring-explainer"><div className="object-visual"><img src="./images/cup-and-pin.jfif" alt="A VEX Cup representing food and medicine beside a Pin representing tools and medical supplies" /></div><div className="scoring-copy"><span className="eyebrow">Emergency supply scoring</span><h2>Deliver more vital supplies.</h2><p>Total Score = Cups × 5 + Pins × 10</p></div><div className="score-values"><div><span>Cup</span><strong>5</strong><small>points</small></div><div><span>Pin</span><strong>10</strong><small>points</small></div></div></section>
  ...
  return (
    <main id="main-content" className="match-page"><section ref={matchArea} className={`match-console three-team-match ${alert ? `has-${alert.type}-alert` : ''}`}>
      ...
      <div className="match-main-grid"><section className="match-timer" aria-live="polite"><span className="current-driver-label">All teams · Driver {matchTimer.currentDriver + 1}</span><h2>DRIVE</h2><strong className="giant-time">{matchTimer.display}</strong><TimerControls timer={{ ...matchTimer, start: onMatchStart, pause: onMatchPause }} onReset={onMatchReset} /></section>
        <section className="multi-score-console"><div className="score-heading"><div><span className="eyebrow">Live scoring</span><h2>Three delivery stations</h2></div><button type="button" className="reset-score" onClick={onScoresReset}>Reset all</button></div><div className="team-score-stack">{project.teams.map((team, teamIndex) => {
      ...
      </div><footer className="match-footer"><span className="mission-reminder"><b>Emergency mission:</b> Neutral Zone → Each team’s Delivery Zone</span><button type="button" className="end-save" onClick={onSave}>End and Save Match</button></footer>
    </section></main>
  );
}
